...
//    }
     ...
     * @param day   int
     */
    private void gregorianToPersian(int year, int month, int day) {
...
//    }
    ...
    public String ConvertToPersianDate(String date) {
        String[] split_one = date.split("-", 3);
        String[] split_two = split_one[2].split("T", 2);
        this.gregorianToPersian(Integer.parseInt(split_one[0]), Integer.parseInt(split_one[1]), Integer.parseInt(split_two[0]));
        String sb = this.getYear() +
                "-" +
                this.getMonth() +
                "-" +
                this.getDay();
        return new Arrange().safelyPersianConvert(sb);

    }


    public String getClock(String clock){
        String[] split_one = clock.split("T", 2);
        StringBuilder sb=new StringBuilder();
        for(int i=0;i<5;i++)
            sb.append(split_one[1].charAt(i));
        return new Arrange().safelyPersianConvert(sb.toString());
    }
